...
def calc_loss(batch, net, tgt_net, device="cpu"):
    ...
    next_state_values[done_mask] = 0.0              # if this is not done, training will not converge
    next_state_values = next_state_values.detach()  # target network is not trained
...
def main():
    ...
    while True:
        ...
        if reward is not None:
            ...
            # log
            print("Frame: %d\tDone games: %d\tMean reward: %.3f\tepsilon: %.3f\tFPS: %.2f" %
